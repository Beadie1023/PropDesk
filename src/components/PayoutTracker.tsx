...
export function PayoutTracker({ accounts, trades }: { accounts: Account; trades: Trade }) {
 ...
 => accounts.map((a) => estimatePayout(a, trades)) [accounts, trades],
 );
 ...
 <div className="stat-value text-lg font-bold text-slate-100">On-Demand</div>
 </div>
 </div>
...
function PayoutAccountCard({
 ...
 const avgDailyNetProfit =
 daysCompleted > 0 ? (estimate?.netProfit ?? 0) / daysCompleted : 0;
 ...
 (t) => t.account_name === account.name && t.trade_date === today,
 );
 ...
 <span
 className={`h-2 w-2 rounded-full ${ACCOUNT_COLORS[account.name] ?? 'bg-accent-500'}`}
 />
 ...
 <span
 className={`text-[11px] stat-value ml-auto ${
 ACCOUNT_DOT[account.name] ?? 'text-accent-400'
 }`}
 >
 ...
 {formatCurrency(estimate?.splitAmount ?? 0)}
 </span>
 </div>
 </div>
 ...
 style={{
 width: `${Math.min((daysCompleted / account.minTradingDays) * 100, 100)}%`,
 }}
 />
 </div>
 {daysToMin > 0 && (
 ...
 {Array.isArray(referencePeriod.cycleEndDate)
 ? referencePeriod.cycleEndDate[0]
 : referencePeriod.cycleEndDate}
 </span>
 </div>
 <div className="flex justify-between">
 ...
 {firstWithdrawal.eligible
 ? 'Unlocked'
 : `Locked (${firstWithdrawal.daysRemaining} days left)`}
 </span>
 </div>
 ...
 <span
 className={`font-semibold ${todayProfit >= 0 ? 'text-bull-400' : 'text-bear-400'}`}
 >
 {todayProfit >= 0 ? '+' : ''}
 {formatCurrency(todayProfit)} ({todayWinRate.toFixed(0)}% WR)
 </span>
 </div>
 </div>
 </div>
